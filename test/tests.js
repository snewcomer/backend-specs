...
var url = process.env.URL || 'http://localhost:8000/tickets';
...
describe('Create Ticket Item', function() {
  var result;

  before(function() {
    result = post(url, { request: 'Walk the dog' });
  });
  ...
  it('should receive a location hyperlink', function() {
    return assert(result, 'header.location').to.match(/^https?:\/\/.+\/tickets\/[\d]+$/);
  });
  ...
  it('should create the item', function() {
    ...
    return assert(item, "body.request").that.equals('Walk the dog');
  });
...
describe('Update Ticket Item', function() {
  var location;

  beforeEach(function(done) {
    post(url, {request: 'Walk the dog'}).then(function(res) {
...
describe('Delete Ticket Item', function() {
  var location;
